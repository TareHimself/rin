...
public interface IGraphConfig
{
    ...
    public uint ReadBuffer(uint id, BufferStage stage) => UseBuffer(id,stage,ResourceUsage.Read);
    public uint WriteBuffer(uint id, BufferStage stage) => UseBuffer(id,stage,ResourceUsage.Write);
    
    public uint ReadImage(uint id, ImageLayout layout) => UseImage(id,layout,ResourceUsage.Read);
    public uint WriteImage(uint id, ImageLayout layout) => UseImage(id,layout,ResourceUsage.Write);
