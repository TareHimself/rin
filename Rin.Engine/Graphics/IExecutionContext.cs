...
public interface IExecutionContext
{
    ...
    {
        return Dispatch((uint)float.Ceiling(x / (float)computeShader.GroupSizeX), y: (uint)float.Ceiling(y / (float)computeShader.GroupSizeY),
            z: (uint)float.Ceiling(z / (float)computeShader.GroupSizeZ));
    }
    ...
    public IExecutionContext DrawIndexedIndirectCount(IDeviceBufferView commands,IDeviceBufferView drawCount,uint maxDrawCount, uint stride,uint commandsOffset = 0, uint drawCountOffset = 0);
