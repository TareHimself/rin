...
{
    private List<Pair<ulong, ulong>> _writes = [];
    ...
    public ulong GetAddress() => buffer.GetAddress();

    public IDeviceBufferView GetView(ulong offset, ulong size) => new DeviceBufferView(this, offset, size);
    ...
    public unsafe void Write(void* src, ulong size, ulong offset = 0)
    {
        ...
        {
            if((write.First <= begin && write.Second <= begin) || (write.First >= end && write.Second >= end)) continue;
