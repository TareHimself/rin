...
public class DefaultFontManager : IFontManager
{
    ...
    readonly struct CacheKey( bool isBold, bool isItalic,char character, string fontName)
        : IEquatable<CacheKey>
    {
        ...
        {
            return _isBold == other._isBold && _isItalic == other._isItalic && _character == other._character && _fontName == other._fontName;
        }
    ...
    private readonly FontCollection _collection = new FontCollection();
    private readonly ConcurrentDictionary<CacheKey,GlyphInfo> _atlases = [];
    private readonly GlyphInfo _defaultGlyph = new GlyphInfo()
    {
        AtlasId = -1,
    ...
    public void Prepare(FontFamily fontFamily,ReadOnlySpan<char> characters,FontStyle style = FontStyle.Regular)
    {
        ...
        foreach (var (_,key) in toGenerate)
        {
            _atlases.AddOrUpdate(key,pending,(k,i) => pending);
        }
        ...
        Task.Run(() =>
        {
            try
            {
                ...
                var results = (IEnumerable<Pair<Result,int>>)renderers.Select((c,idx) => new Pair<Result?,int>(c.Generate(3f, GetPixelRange()),idx)).Where(c => c.First != null);
                ...
                var generated = results.Select(c =>
                {
                    ...
                    return textureManager.CreateTexture(data,
                        new Extent3D((uint)result.PixelWidth, (uint)result.PixelHeight), ImageFormat.RGBA8,tiling: ImageTiling.ClampEdge).Then(
                        id =>
                        {
                            glyph.AtlasId = id;
                            data.Dispose();
    ...
    public void Dispose()
    {
        ...
        SGraphicsModule.Get().GetTextureManager().FreeTextures(_atlases.Select(c => c.Value.AtlasId).Where(c => c != -1).ToArray());
        _atlases.Clear();
    }

    public float GetPixelRange() => 12.0f;
}