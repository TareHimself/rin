...
public class ForwardRenderingPass(CameraComponent camera, Vec2<uint> size, CollectScenePass? collectPass = null) : IPass
{
    ...
    public void Configure(IGraphConfig config)
    {
        ...
        var materialBufferSize = _collectPass.Geometry.Aggregate((ulong)0,
            (total, geometryDrawCommand) => total + geometryDrawCommand.Material.GetRequiredMemory());
    ...
    public void Execute(ICompiledGraph graph, Frame frame, VkCommandBuffer cmd)
    {
        ...
        cmd.ImageBarrier(OutputImage, ImageLayout.Undefined, ImageLayout.General)
            .ClearColorImages(0.0f, ImageLayout.General, OutputImage)
        ...
        var colorAttachment = OutputImage.MakeColorAttachmentInfo(0.0f);
        ...
        sceneBuffer.Write(new SceneInfo
        {
            ...
            ViewProjection = sceneFrame.Projection,
                 ...
                 }))
        {
            ...
            var size = first.Material.GetRequiredMemory() * (ulong)infos.Length;
            var view = materialBuffer?.GetView(materialBufferSize, size);
            materialBufferSize += size;
            first.Material.Execute(sceneFrame, view, infos);
        }
