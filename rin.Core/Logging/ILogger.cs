...
{
    abstract ILogger AddTransport(ITransport transport);

    abstract IEnumerable<ITransport> GetTransports();
    ...
    ILogger Error(string message,Exception? exception = null)
    {
        ...
        foreach (var transport in GetTransports())
        {
            transport.Error(formatted,exception);
        }

        return this;
    }

    string FormatMessage(string severity, string message) => message;
}