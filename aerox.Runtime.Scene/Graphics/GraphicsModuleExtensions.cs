...
{
    public static DeviceGeometry NewGeometry<T>(this GraphicsModule subsystem, T[] vertices, uint[] indices)
        where T : struct
    {
        ...
        var vertexBuffer = subsystem.GetAllocator().NewBuffer(verticesByteSize,
            ...
            VkMemoryPropertyFlags.VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, true, debugName: "Vertex Buffer");
        ...
        VkBufferDeviceAddressInfo addressInfo = new VkBufferDeviceAddressInfo()
        {
            sType = VkStructureType.VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO,
            buffer = vertexBuffer
        };
        ...
        var indexBuffer = subsystem.GetAllocator().NewBuffer(indicesByteSize,
            ...
            VkBufferUsageFlags.VK_BUFFER_USAGE_TRANSFER_DST_BIT,
            VkMemoryPropertyFlags.VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, true, debugName: "Vertex Index Buffer");
