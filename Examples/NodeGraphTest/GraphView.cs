...
public class GraphView : MultiSlotCompositeView<GraphSlot>, IGraphView
{
    ...
    private NodeDragInfo? _currentDragInfo;
    ...
    public override void Collect(in Matrix4x4 transform, in Rect2D clip, CommandList commands)
    {
        ...
        {
            var inputPos = input.GetPinAbsolutePosition();
            var outputPos = output.GetPinAbsolutePosition();
            var offset = ComputeHandleLength(inputPos, outputPos);
            commands.AddCubicCurve(Matrix4x4.Identity, inputPos, outputPos,
                inputPos + new Vector2(-offset, 0),
                outputPos + new Vector2(offset, 0), 5, Color.Green);
        }

        {
            if (_pinConnectionRequest is { } request)
            {
                ...
                var requesterPos = request.Requester.GetPinAbsolutePosition();
                var offset = ComputeHandleLength(requesterPos, request.Position);
                commands.AddCubicCurve(Matrix4x4.Identity, requesterPos, request.Position,
                    requesterPos + new Vector2(requesterIsInput ? -offset : offset, 0),
                    request.Position + new Vector2(!requesterIsInput ? -offset : offset, 0), 5, Color.Green);
            }
        }
    ...
    /// <summary>
    ///     A fixed handle length makes long connections (nodes placed far apart) render as visually
    ///     straight lines - the control points end up almost collinear with the endpoints. Scale the
    ///     handle with the horizontal distance between the pins instead, like blueprint-style editors do,
    ///     so the curve stays visible regardless of how far apart the nodes are.
    /// </summary>
    private static float ComputeHandleLength(in Vector2 from, in Vector2 to)
    {
        return Math.Max(Math.Abs(to.X - from.X) * 0.5f, 50f);
    }
    ...
    {
        if (_pinConnectionRequest is { } request) request.Position = e.Position;
    ...
    public override void OnCursorUp(CursorUpSurfaceEvent e)
    {
        ...
        if (_pinConnectionRequest is { } request && Surface is not null)
        {
            ...
            if (connectionEvent.Handled)
            {
                ...
                if (_connections.ContainsKey(input)) _connections.Remove(input);
    ...
    public override bool Add(GraphSlot slot)
    {
