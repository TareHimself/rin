...
namespace Rin.Core.Shared.Buffers;
    ...
    where T : unmanaged
{
    ...
    public Buffer(int elements)
    {
        ...
        _ptr = Native.memoryAllocate(Utils.ByteSizeOf<T>(elements));
    }
    ...
    public Buffer(IntPtr data, ulong byteSize, bool copy = false)
    {
        ...
        if (copy)
        {
            _ptr = Native.memoryAllocate(byteSize);
            Write(data, byteSize);
        }
        else
        {
    ...
    public Buffer(ReadOnlySpan<T> elements)
    {
        ...
        unsafe
        {
            _ptr = new IntPtr(Native.memoryAllocate(Utils.ByteSizeOf<T>(elementCount)));
    ...
    public void BinaryDeserialize(Stream input)
    {
        unsafe
        {
            if (_ptr != IntPtr.Zero)
            {
                Native.memoryFree(_ptr);
                _ptr = IntPtr.Zero;
                ElementCount = 0;
            }

            var byteSize = (nuint)input.ReadUInt64();
            _ptr = new IntPtr(Native.memoryAllocate(byteSize));
    ...
    public void Zero()
    {
        Native.memorySet(_ptr, 0, GetByteSize());
    }
    ...
    {
        if (_ptr != IntPtr.Zero) Native.memoryFree(_ptr);
        _ptr = IntPtr.Zero;
    }
