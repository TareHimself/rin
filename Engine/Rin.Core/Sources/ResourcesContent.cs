...
using System.Text;
using Rin.Core.Sources.Exceptions;

namespace Rin.Core.Sources;
...
public class AssemblyResource : ISource
{
    ...
    public AssemblyResource(Assembly assembly, string alias, string? rootAssemblyPath = null)
    {
        ...
        _assemblyPath = new StringBuilder().Append(assemblyName).Append('.')
            .Append(string.Join('.', rootPathStr.Split('/', StringSplitOptions.RemoveEmptyEntries))).Append(suffix)
            .ToString();
        BasePath = alias;
    }
    ...
    {
        var name = resourceName[_assemblyPath.Length..]
            .Split('.',
                StringSplitOptions
                    .RemoveEmptyEntries); //; resourceName[(_assemblyName.Length + _assemblySuffix.Length)..].Split('.');
