...
TransformInfo::TransformInfo(const WidgetSurface* inSurface) : TransformInfo(
    Matrix3<float>{1.0f}, inSurface->GetDrawSize().Cast<float>())
{
}

TransformInfo::TransformInfo(const Shared<WidgetSurface>& inSurface) : TransformInfo(
    Matrix3<float>{1.0f}, inSurface->GetDrawSize().Cast<float>())
{
}

TransformInfo::TransformInfo(const Shared<Widget>& widget, bool absolute) : TransformInfo(
    absolute ? widget->ComputeAbsoluteTransform() : widget->ComputeRelativeTransform(), widget->GetSize())
{
}
