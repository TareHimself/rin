...
void WidgetWindowSurface::Init()
{
    ...
    _cursorMoveHandle = window->onCursorMoved->Add<WidgetWindowSurface>(
...
void WidgetWindowSurface::HandleBeforeDraw(Frame* frame)
{
    if (auto image = GetDrawImage())
    {
        image->Barrier(frame->GetCommandBuffer(), vk::ImageLayout::eUndefined,
...
void WidgetWindowSurface::HandleAfterDraw(Frame* frame)
{
    if (auto image = GetDrawImage())
    {
        image->Barrier(frame->GetCommandBuffer(), vk::ImageLayout::eColorAttachmentOptimal,
...
void WidgetWindowSurface::OnWindowCursorButton(Window* window, CursorButton button, InputState state)
{
    ...
    {
        NotifyCursorDown(newShared<CursorDownEvent>(this->GetSharedDynamic<WidgetWindowSurface>(), button,
                                                    GetCursorPosition()));
    }
    else if (state == InputState::Released)
    {
        NotifyCursorUp(newShared<CursorUpEvent>(this->GetSharedDynamic<WidgetWindowSurface>(), button,
                                                GetCursorPosition()));
    }
}
