...
public:
    ...
    virtual void DrawCommandsToFinalCommands(SurfaceFrame* frame, const std::vector<CommandInfo>& drawCommands,
    ...
    std::enable_if_t<std::is_constructible_v<T, TArgs...> && std::is_base_of_v<Widget, T>, Shared<T>> AddChild(
        TArgs&&... args);
};
