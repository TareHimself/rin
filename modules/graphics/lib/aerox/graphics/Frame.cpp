...
namespace aerox::graphics
{
    Frame::Frame(WindowRenderer* renderer)
    {
        ...
        _commandPool = device.createCommandPool({vk::CommandPoolCreateFlagBits::eResetCommandBuffer,graphicsModule->GetQueueFamily()});
        ...
        //_allocator = DescriptorAllocator::New(1024,{});
    }
    ...
    {
        const auto device = _renderer->GetModule()->GetDevice();
        if(const auto r = device.waitForFences({_renderFence},true,std::numeric_limits<uint64_t>::max()); r != vk::Result::eSuccess)
        {
