...
namespace aerox::graphics
{
    ...
    std::vector<uint32_t> ShaderManager::CompileAstToSpirv(const std::string& id,vk::ShaderStageFlagBits stage,
                                                           const std::shared_ptr<ashl::ModuleNode>& node)
    {
    ...
    {
        if(stage & vk::ShaderStageFlagBits::eVertex) return glslang_stage_t::GLSLANG_STAGE_VERTEX;

        if(stage & vk::ShaderStageFlagBits::eFragment) return glslang_stage_t::GLSLANG_STAGE_FRAGMENT;
