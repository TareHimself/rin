...
void WindowRenderer::Draw()
{
    ...
    vk::SemaphoreSubmitInfo signalSemaphoreSubmits[] = {
        vk::SemaphoreSubmitInfo{frame->GetRenderSemaphore(), 1, vk::PipelineStageFlagBits2::eAllGraphics}
    };
    vk::SemaphoreSubmitInfo waitSemaphoreSubmits[] = {
        vk::SemaphoreSubmitInfo{frame->GetSwapchainSemaphore(), 1, vk::PipelineStageFlagBits2::eColorAttachmentOutput}
    };

    try
    {
        _graphicsModule->GetQueue().submit2(
            vk::SubmitInfo2({}, waitSemaphoreSubmits, cmdSubmits, signalSemaphoreSubmits), frame->GetRenderFence());
