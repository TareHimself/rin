...
Shared<DeviceBuffer> Allocator::NewBuffer(vk::DeviceSize size, vk::BufferUsageFlags usageFlags,
                                          vk::MemoryPropertyFlags propertyFlags, bool sequentialWrite, bool preferHost,
                                          bool mapped,
...
Shared<DeviceBuffer> Allocator::NewTransferBuffer(const vk::DeviceSize size, const bool sequentialWrite,
                                                  const std::string& debugName)
{
    return NewBuffer(size, vk::BufferUsageFlagBits::eTransferSrc | vk::BufferUsageFlagBits::eTransferDst,
                     vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
                     sequentialWrite, true, true, debugName);
}

Shared<DeviceBuffer> Allocator::NewUniformBuffer(vk::DeviceSize size, bool sequentialWrite,
                                                 const std::string& debugName)
{
...
Shared<DeviceBuffer> Allocator::NewStorageBuffer(vk::DeviceSize size, bool sequentialWrite,
                                                 const std::string& debugName)
{
    return NewBuffer(size, vk::BufferUsageFlagBits::eStorageBuffer, vk::MemoryPropertyFlagBits::eHostVisible,
                     sequentialWrite, false, true, debugName);
}
...
Shared<DeviceImage> Allocator::NewImage(const vk::ImageCreateInfo& createInfo, const std::string& debugName)
{
    ...
    return newShared<DeviceImage>(_allocator, vmaImage, alloc, createInfo.extent,
                                  vulkanFormatToImageFormat(createInfo.format));
}
