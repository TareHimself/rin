...
GraphicsShader::GraphicsShader(ShaderManager* manager, const Shared<rsl::ModuleNode>& inVertex,
                               const Shared<rsl::ModuleNode>& inFragment) : Shader(manager)
{
...
std::shared_ptr<rsl::ModuleNode> GraphicsShader::GetVertexShader() const
{
    return _vertexShader;
}

std::shared_ptr<rsl::ModuleNode> GraphicsShader::GetFragmentShader() const
{
...
Shared<GraphicsShader> GraphicsShader::FromFile(const std::filesystem::path& path)
{
    ...
    const auto fullPath = absolute(path).string();
    if (_shaders.contains(fullPath)) return _shaders[fullPath];
    auto manager = GraphicsModule::Get()->GetShaderManager();
    TokenList nodes{};
    std::shared_ptr<ModuleNode> vertex{};
    std::shared_ptr<ModuleNode> fragment{};
    try
    {
        nodes = tokenize(fullPath);
        const auto ast = parse(nodes);
        vertex = extractScope(ast, EScopeType::Vertex);
        fragment = extractScope(ast, EScopeType::Fragment);
    ...
    {
        std::cout << "Failed to parse shader " << e.what() << std::endl;
        throw;
    }
