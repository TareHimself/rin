...
void GraphicsModule::InitVulkan(Window* window)
{
    ...
    _immediateCommandPool = _device.createCommandPool(
        {vk::CommandPoolCreateFlagBits::eResetCommandBuffer, _queueFamily});
...
bool GraphicsModule::IsDependentOn(RinModule* module)
{
...
{
    if (auto result = _device.waitForFences(_immediateFence, true, std::numeric_limits<uint64_t>::max()); result !=
        vk::Result::eSuccess)
    {
...
Shared<DeviceImage> GraphicsModule::CreateImage(const unsigned char* data, const vk::Extent3D& extent,
                                                ImageFormat format, vk::ImageUsageFlags usage, bool mipMap,
