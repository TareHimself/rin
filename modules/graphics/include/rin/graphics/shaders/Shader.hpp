...
public:
    ...
    void ComputeResources(const std::vector<std::pair<vk::ShaderStageFlags,std::shared_ptr<rsl::ModuleNode>>>& shaders);

    static vk::ShaderStageFlags ScopeTypeToStageFlags(const rsl::EScopeType& scopeType);
    ...
    // Shader(const Shared<rsl::ModuleNode>& inModule,rsl::EScopeType inScopeType);
    // Shader(const Shared<rsl::ModuleNode>& inModule,rsl::EScopeType inScopeType,const std::string& inId);
};
