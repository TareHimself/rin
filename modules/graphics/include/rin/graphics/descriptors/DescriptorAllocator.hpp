...
public:
    ...
    Shared<DescriptorSet> Allocate(const vk::DescriptorSetLayout& layout,
                                   const std::vector<uint32_t>& variableCount = {});

    static Shared<DescriptorAllocator> New(uint32_t maxSets, const std::vector<PoolSizeRatio>& poolRatios,
                                           const vk::DescriptorPoolCreateFlags& poolCreateFlags =
