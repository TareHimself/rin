...
#include "ClassFunctionDelegate.hpp"
#include "SharedClassFunctionDelegate.hpp"
#include "Delegate.hpp"
#include "FunctionDelegate.hpp"
#include "rin/core/macros.hpp"
class _DelegateList;
    ...
    class _DelegateList : public Disposable
    {
    public:
    ...
    DelegateListHandle DelegateList<TArgs...>::Add(const Shared<Delegate<void, TArgs...>>& delegate)
    {
        if(_delegates.empty())
        {
            ...
            return DelegateListHandle{0,this->GetWeakStatic<_DelegateList>()};
        }

        auto id = _delegates.rbegin()->first + 1;
        _delegates.emplace(id,delegate);
        return DelegateListHandle{id,this->GetWeakStatic<_DelegateList>()};
    }
