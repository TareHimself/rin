...
public class SdfArchive : IDisposable, ISdfContainer
{
    ...
    {
        if (_archive.CreateReadStream($"{id}.png") is { } stream) return HostImage.Create(stream);
    ...
    {
        foreach (var vector in vectors) _vectors.AddOrUpdate(vector.Id, vector, (_, _) => vector);
        WriteInfo();
    }
    ...
    private void LoadFromArchive()
    {
        ...
        if (_archive.Keys.Contains("info.json") && _archive.CreateReadStream("info.json") is { } stream &&
            JsonSerializer.Deserialize<Info>(stream) is { } info)
        {
            foreach (var image in info.Images) _images.AddOrUpdate(image.Id, image, (_, _) => image);

            foreach (var vector in info.Vectors) _vectors.AddOrUpdate(vector.Id, vector, (_, _) => vector);
        }
    }
