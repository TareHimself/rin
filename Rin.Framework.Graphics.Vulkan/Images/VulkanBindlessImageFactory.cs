...
public class VulkanBindlessImageFactory : IBindlessImageFactory
{
    ...
    private readonly DescriptorAllocator _allocator = new(DescriptorTotal, [
        new PoolSizeRatio(DescriptorType.SampledImage,
            (float)(MaxTextures + MaxCubemaps + MaxTextureArrays) / DescriptorTotal),
        ...
        lock (_sync)
        {
            ...
            foreach (var handle in handles)
            {
                ...
                switch (handle.Type)
                {
                    ...
                    {
                        _descriptorSet.WriteSampledImage(TexturesBinding, _defaultTexture, ImageLayout.ShaderReadOnly,
                            (uint)handle.Id);
                        var resource = _textures[handle.Id];
                    ...
                    {
                        _descriptorSet.WriteSampledCubemap(TexturesBinding, _defaultCubemap, ImageLayout.ShaderReadOnly,
                            (uint)handle.Id);
                        var resource = _cubemaps[handle.Id];
                        if (resource.State == BindlessResourceState.Uploading)
                        {
                    ...
                    {
                        _descriptorSet.WriteSampledImageArray(TexturesBinding, _defaultTextureArray,
                            ImageLayout.ShaderReadOnly, (uint)handle.Id);
                        var resource = _textureArrays[handle.Id];
                        if (resource.State == BindlessResourceState.Uploading)
                        {
                            _pendingTasks[handle].SetCanceled();
        ...
        foreach (var handle in handles)
            switch (handle.Type)
            {
                case ImageType.Texture:
                {
                    ...
                    _descriptorSet.WriteSampledImage(TexturesBinding, resource.Source, ImageLayout.ShaderReadOnly,
                        (uint)handle.Id);
                ...
                case ImageType.Cubemap:
                {
                    ...
                    _descriptorSet.WriteSampledCubemap(CubemapsBinding, resource.Source, ImageLayout.ShaderReadOnly,
                        (uint)handle.Id);
                ...
                case ImageType.TextureArray:
                {
                    ...
                    _descriptorSet.WriteSampledImageArray(TexturesBinding, resource.Source, ImageLayout.ShaderReadOnly,
                        (uint)handle.Id);
    ...
    private Task HandleAsyncBindless<TBindlessResource>(out ImageHandle handle, List<TBindlessResource> list,
