...
public class Token(TokenType type, string value, DebugInfo debugInfo) : IFormattable
{
    ...
    {
        return $"Token({nameof(Value)}: {Value}, {nameof(Type)}: {Type}, {nameof(DebugInfo)}: {DebugInfo})";
    }
}