   ...
   public static TokenList Run(string fileName, Stream content)
   {
...

      {
         ...
         while (reader.ReadLine() is { } line)
         {
            ...
            {
               rawTokens.InsertBack(new Token(col.ToString(), new DebugInfo(fileName, lineNo, colNo,lineNo,colNo + 1)));
               colNo++;
            }
      ...
      while (rawTokens.NotEmpty())
      {
         ...
         else
         {
            ...
            rawTokens.RemoveFront();
            if (int.TryParse(combinedString, out var _))
            {
               while (rawTokens.NotEmpty())
               {
                  ...
                  if (rawTokens.Front().Value == ".")
                  {
                     ...
                     if(float.TryParse(combinedString + point.Value + next.Value, out var _))
                     {
                  ...
                  if (rawTokens.Front().Value == "e")
                  {
                     ...
                     if (rawTokens.Front().Type == TokenType.OpSubtract)
                     {
                        ...
                        var next = rawTokens.Front();
                        if(float.TryParse(combinedString + exp.Value + sign.Value + next.Value, out var _))
                        {
                     ...
                     {
                        var next = rawTokens.Front();
                        if(float.TryParse(combinedString + exp.Value + next, out var _))
                        {
                  ...
                  if (int.TryParse(rawTokens.Front().Value, out var _))
                  {
