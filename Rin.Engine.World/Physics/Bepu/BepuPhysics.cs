...
public class BepuPhysics : IPhysicsSystem
{
    ...
    public void Dispose()
    {
        foreach (var (_, body) in _bodies) body.Dispose();
        Simulation.Dispose();
        _pool.Clear();
    }
    ...
    public void Update(double deltaTime)
    {
        lock (Simulation)
        {
            ...
            foreach (var (comp, body) in bodies)
            {
                ...
                comp.SetTransform(transform, Space.World);
            }
        }
    }
    ...
    public void Start()
    {
        Simulation.Statics.Add(
            new StaticDescription(new Vector3(0, -7, 0), Simulation.Shapes.Add(new Box(200, 5, 200))));
    }
    ...
    public RayCastResult? RayCast(Vector3 begin, Vector3 direction, float distance, int channel)
    {
        ...
        Simulation.RayCast(begin, direction, distance, ref hitHandler, channel);
    ...
    {
        if (Simulation.Statics[reference.StaticHandle].Exists) return _staticBodies[reference.StaticHandle];

        if (Simulation.Bodies[reference.BodyHandle].Exists) return _dynamicBodies[reference.BodyHandle];

        throw new KeyNotFoundException($"The body {reference.StaticHandle} does not exist.");
    }

    private struct NarrowPhaseCallbacks(BepuPhysics system) : INarrowPhaseCallbacks
    {
            ...
            out PairMaterialProperties pairMaterial) where TManifold : unmanaged, IContactManifold<TManifold>
        {
            pairMaterial = new PairMaterialProperties
                { FrictionCoefficient = 1, MaximumRecoveryVelocity = 2, SpringSettings = new SpringSettings(1, 1) };
    ...
    private struct PoseIntegratorCallbacks(BepuPhysics system) : IPoseIntegratorCallbacks
    {
        ...
        public void PrepareForIntegration(float dt)
        {
            ...
            _gravityAcceleration = Vector3Wide.Broadcast(system.Gravity);
        }
    ...
    private class RayHitHandler(BepuPhysics physics, int channel) : IRayHitHandler
    {
