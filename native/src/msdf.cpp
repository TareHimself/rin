...
void msdfFreeContext(void* context)
{
    delete static_cast<GlyphContext*>(context);
}

void msdfGlyphContextMoveTo(void* context, Vec2* to)
{
...
void msdfGlyphContextLineTo(void* context, Vec2* to)
{
    auto ctx = static_cast<GlyphContext*>(context);
...
void msdfGlyphContextQuadraticBezierTo(void* context, Vec2* control, Vec2* to)
{
    auto ctx = static_cast<GlyphContext*>(context);
    msdfgen::Point2 endpoint = *to;
    if (endpoint != ctx->position) {
...
void msdfGlyphContextCubicBezierTo(void* context, Vec2* control1, Vec2* control2, Vec2* to)
{
    auto ctx = static_cast<GlyphContext*>(context);
    msdfgen::Point2 endpoint = *to;
...
void msdfEndContext(void* context)
{
    auto ctx = static_cast<GlyphContext*>(context);
...
void msdfRenderContext(void* context,RenderMsdfCallback callback)
{
    auto ctx = static_cast<GlyphContext*>(context);
    ...
    msdfgen::generateMSDF(msdf,shape,projection,30.0);
