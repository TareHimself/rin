...
using Rin.Framework.Graphics.Vulkan.Shaders;
    ...
    FrozenDictionary<uint, DescriptorSet>? globalSets = null) : IExecutionContext
{
    ...
    public IExecutionContext ClearStencilImages(uint clearValue, params ITexture[] images)
    {
    ...
    public IExecutionContext ClearDepthImages(float clearValue, params ITexture[] images)
    {
        Debug.Assert(images.All(c => c.Format == ImageFormat.Depth));
        Debug.Assert(images.All(c => c is IVulkanTexture));
        CommandBuffer.ClearDepthImages(clearValue, ImageLayout.General, Unsafe.As<IVulkanTexture[]>(images));
        return this;
    }
