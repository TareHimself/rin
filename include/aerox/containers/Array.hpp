...
public:
  ...
  template<typename  = std::enable_if_t<std::equality_comparable_with<T,T>>>
  std::optional<uint64_t> index_of(T data);
  std::optional<uint64_t> index_of(std::function<bool(T &a)> fn);
...
template <typename T> template <typename> std::optional<uint64_t> Array<T>::
index_of(T data) {
  for (auto i = 0; i < this->size(); i++) {
    if (data == this->at(i)) {
      return i;
...
template <typename T> std::optional<uint64_t> Array<T>::index_of(std::function<bool(T &a)> fn) {
  for (auto i = 0; i < this->size(); i++) {
    if (comparator(this->at(i))) {
      return i;
    }
  }
