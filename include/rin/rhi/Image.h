...
namespace rin::rhi
{
    ...
    public:
        ...
        Image(const int width, const int height, const int channels, T* data, const bool owned = false)
        {
            _width = width < 0 ? 0 : width;
            _height = height < 0 ? 0 : height;
            _channels = channels < 0 ? 0 : channels;
        ...
        [[nodiscard]] T At(const unsigned int index) const
        {
            return _data[index];
        }
        ...
        void SetChannels(const int channels, const T& fillValue)
        {
