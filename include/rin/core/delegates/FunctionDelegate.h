...
namespace rin
{
    ...
    template <typename TReturn, typename... TArgs>
    Shared<FunctionDelegate<TReturn, TArgs...>> newFunctionDelegate(const std::function<TReturn(TArgs...)>& func)
    {
        return std::make_shared<FunctionDelegate<TReturn, TArgs...>>(func);
    }

    template <typename TReturn, typename... TArgs>
    Shared<FunctionDelegate<TReturn, TArgs...>> newFunctionDelegate(TReturn (func)(TArgs...))
    {
        return newFunctionDelegate(std::function(func));
    }

    template <typename TReturn, typename... TArgs>
    Shared<FunctionDelegate<TReturn, TArgs...>> newFunctionDelegate(std::function<TReturn(TArgs...)>&& func)
    {
        return std::make_shared<FunctionDelegate<TReturn, TArgs...>>(func);
    }
}