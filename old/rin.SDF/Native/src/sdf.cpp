...
_declspec(dllexport) void msdfGenerateMSDF(GlyphContext* context,float angleThreshold,float pixelRange, RenderMsdfCallback callback)
{
...
_declspec(dllexport) void msdfGenerateMTSDF(GlyphContext* context,float angleThreshold,float pixelRange, RenderMsdfCallback callback)
{
    if(!context->hasContent) {    
        return;
    }
