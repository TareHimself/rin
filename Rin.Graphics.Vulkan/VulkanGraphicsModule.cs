...
using Rin.Graphics.Vulkan.Shaders.Slang;
using Rin.Graphics.Vulkan.Windows;
...
public partial class VulkanGraphicsModule : IGraphicsModule
{
    ...
    {
        Native.platformWindowPump();

        unsafe
        {
            var events = stackalloc Native.Window.WindowEvent[_maxEventsPerPeep];
            int eventsPumped;
            do
            {
                eventsPumped = Native.platformWindowGetEvents(events, _maxEventsPerPeep);
    ...
    {
        if (!_windows.TryGetValue(window, out var windowRenderer)) return;

        lock (_renderers)
        {
            _renderers.Remove(windowRenderer);
        }
    ...
    private IWindow Internal_CreateWindow(string name, in Extent2D extent, WindowFlags flags = WindowFlags.None,
        IWindow? parent = null)
    {
        var handle = Native.platformWindowCreate(name, extent, flags);
    ...
    /// <returns></returns>
    private Task Sync(Action action)
    {
        ...
        bool sequentialWrite = true, bool preferHost = false, bool mapped = false, string debugName = "Buffer")
    {
        unsafe
        {
            ...
            Native.allocatorNewBuffer(&buffer, ref allocation, size, _allocator, sequentialWrite ? 1 : 0,
                preferHost ? 1 : 0,
        ...
        ReadOnlySpan<VkSemaphoreSubmitInfo> signalSemaphores, ReadOnlySpan<VkSemaphoreSubmitInfo> waitSemaphores)
    {
        unsafe
        {
            fixed (VkSemaphoreSubmitInfo* pWaitSemaphores = waitSemaphores)
            {
                fixed (VkSemaphoreSubmitInfo* pSignalSemaphores = signalSemaphores)
                {
                    fixed (VkCommandBufferSubmitInfo* pCommandBuffers = commandBuffers)
                    {
                        var submit = new VkSubmitInfo2
                        {
                            ...
                            signalSemaphoreInfoCount = (uint)signalSemaphores.Length,
                            pWaitSemaphoreInfos = pWaitSemaphores,
                            waitSemaphoreInfoCount = (uint)waitSemaphores.Length
                        };
    ...
    private VkImageView CreateImageView(VkImageViewCreateInfo createInfo)
    {
        unsafe
        {
            VkImageView view;
        ...
        ImageUsage usage = ImageUsage.None)
    {
        unsafe
        {
            ...
            VkImage image = new();
            var allocation = IntPtr.Zero;
            Native.allocatorNewImage(ref image, ref allocation, &imageCreateInfo, _allocator, "Texture");
