...
public class GraphView : MultiSlotCompositeView<GraphSlot>, IGraphView
{
    ...
    public override void Collect(in Matrix4x4 transform, in Rect2D clip, CommandList commands)
    {
        ...
        foreach (var (input, output) in _connections)
            commands.AddCubicCurve(Matrix4x4.Identity, input.GetPinAbsolutePosition(), output.GetPinAbsolutePosition(),
                input.GetPinAbsolutePosition() + new Vector2(-offset, 0),
                output.GetPinAbsolutePosition() + new Vector2(offset, 0), 5, Color.Green);

        {
            if (_pinConnectionRequest is { } request)
            {
                ...
                commands.AddCubicCurve(Matrix4x4.Identity, request.Requester.GetPinAbsolutePosition(), request.Position,
                    request.Requester.GetPinAbsolutePosition() + new Vector2(requesterIsInput ? -offset : offset, 0),
