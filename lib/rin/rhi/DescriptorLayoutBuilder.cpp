...
DescriptorLayoutBuilder& DescriptorLayoutBuilder::AddBinding(uint32_t binding, vk::DescriptorType type,
                                                             const vk::ShaderStageFlags& stageFlags, uint32_t count,
...
{
    auto store = GRuntime::Get()->GetModule<GraphicsModule>()->GetDescriptorLayoutBuilder();
    ...
    if (const auto result = store->Get(createInfo))
    {
        return result;
    }
